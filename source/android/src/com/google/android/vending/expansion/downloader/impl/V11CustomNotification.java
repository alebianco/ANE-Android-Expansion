...
public class V11CustomNotification implements DownloadNotification.ICustomNotification {
    ...
    public Notification updateNotification(Context c) {
        ...
        if (hasPausedText) {
            ...
            int id = c.getResources().getIdentifier("time_remaining_notification", "string", c.getPackageName());
            builder.setContentInfo(c.getString(id, Helpers.getTimeRemaining(mTimeRemaining)));
        }
        if (mIcon != 0) {
            builder.setSmallIcon(mIcon);
        } else {
            int iconResource = c.getResources().getIdentifier("stat_sys_download", "drawable", "android");
            if (hasPausedText) {
                iconResource = c.getResources().getIdentifier("stat_sys_download", "drawable", "android");
            }
